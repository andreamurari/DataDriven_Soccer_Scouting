...
df_latente_a, df_latente_b, df_latente_c = load_databases()

# Extract latent columns dynamically
...
def search_similar_players(player_name, player_season, top_n, max_age, season_filter, same_position, same_league):
    ...
    return target_info, simili[['player', 'age', 'team', 'pos', 'league', 'season', 'Match %']]
...
with st.sidebar:
    ...
    st.divider()
    btn_search = st.button("Find Similar Players", type="primary", use_container_width=True)
...
if btn_search:
    with st.spinner("Scanning the European database..."):
        ...
        if target_data is None:
            ...
            st.info(f"📍 **Role:** {target_data['pos']} | 🛡️ **Team:** {target_data['team']} | 🌍 **League:** {target_data['league']} | 📅 **Season:** {target_data['season']}")
